...
float J;
float V_VSM;
float omegadot_VSM[2];
float omega_VSM[2];
float th_VSM[2] ;
float err_Q[2];
float int_err_Q[2];
float ki_Q = 0.001; //0.01
//float temp_mod = 0.8;
...
//    configureDAC(1); //DAC_NUM
    ...
    LPF_Q = InitiateLPF( Tsamp, 62.8319); // 10 hz
        ...
        J =  2.0 * 0.02 * 500 / omega_nom;// H = 0.02
        D = 50 * 500.0 / omega_nom;
        mP = (1+D)*2*PI*5/5000;
        mQ = 0.05*Vnom/5000;
...
interrupt void adca1_isr(void)
{
   // GpioDataRegs.GPBDAT.bit.GPIO36 = 1; // - to check whether code fits or not
    ...
    PieCtrlRegs.PIEACK.all = PIEACK_GROUP1;
     ...
     INV_CURRENT_p.Ds =  (INV_CURRENT.As*cos(th_VSM[1]) + INV_CURRENT.Bs*cos(th_VSM[1]-ALPHA) + INV_CURRENT.Cs*cos(th_VSM[1]+ALPHA))*0.66666667;
     INV_CURRENT_p.Qs = (-INV_CURRENT.As*sin(th_VSM[1]) - INV_CURRENT.Bs*sin(th_VSM[1]-ALPHA) - INV_CURRENT.Cs*sin(th_VSM[1]+ALPHA))*0.66666667;
     ...
     CAP_VOLTAGE_p.Ds = (CAP_VOLTAGE.As*cos(th_VSM[1]) + CAP_VOLTAGE.Bs*cos(th_VSM[1]-ALPHA) + CAP_VOLTAGE.Cs*cos(th_VSM[1]+ALPHA))*0.66666667;
     CAP_VOLTAGE_p.Qs = (-CAP_VOLTAGE.As*sin(th_VSM[1]) - CAP_VOLTAGE.Bs*sin(th_VSM[1]-ALPHA) - CAP_VOLTAGE.Cs*sin(th_VSM[1]+ALPHA))*0.66666667;
     ...
     GRID_CURRENT_p.Ds =  (GRID_CURRENT.As*cos(th_VSM[1]) + GRID_CURRENT.Bs*cos(th_VSM[1]-ALPHA) + GRID_CURRENT.Cs*cos(th_VSM[1]+ALPHA))*0.66666667;
     GRID_CURRENT_p.Qs = (-GRID_CURRENT.As*sin(th_VSM[1]) - GRID_CURRENT.Bs*sin(th_VSM[1]-ALPHA) - GRID_CURRENT.Cs*sin(th_VSM[1]+ALPHA))*0.66666667;
     ...
     GRID_VOLTAGE_p.Qs = (-GRID_VOLTAGE.As*sin(th_VSM[1]) - GRID_VOLTAGE.Bs*sin(th_VSM[1]-ALPHA) - GRID_VOLTAGE.Cs*sin(th_VSM[1]+ALPHA))*0.66666667;
...
//     th_VSM[1] = th_VSM[0] + omega_nom*Tsamp;
//      if(th_VSM[1]>2*PI)
//      {
//          th_VSM[1] = th_VSM[1]-2.0*PI;
//
//      }
//      else if(th_VSM[1]<0)
//      {
       ...
       if(RELAY==0)//if(RELAY==0)

              {
                  ...
                  V_VSM   =  Vnom;
                   ...
                   omega_VSM[1] = omega_nom  + int_Vgrid_Q[1] + 4.44*Vgrid_Q[1] ;
...

              {
                  ...
                  V_VSM     = Vnom+ mQ*err_Q[1] + int_err_Q[1]; //Vnom - mQ*(Q-Q_REF);

                  omegadot_VSM[1] = ((1+D)*(omega_nom-omega_VSM[1]) - mP*(P - P_REF) )/J;

                  omega_VSM[1] = omega_VSM[0] + 0.5*( omegadot_VSM[0] +  omegadot_VSM[1])*Tsamp;
        ...
        th_VSM[1] = th_VSM[0] + 0.5*(omega_VSM[1] + omega_VSM[0])*Tsamp;
...
//            Vmod_d     = (V_VSM)/(0.5*Vdcnom);
//           Vmod_q     = 0.0 ;

                        Vmod_d     = (VD_REF)/(0.5*Vdcnom);
                        Vmod_q     = 0.0 ;
             ...
             Vmod_alpha = Vmod_d*cos(th_VSM[1])-Vmod_q*sin(th_VSM[1]);
             Vmod_beta  = Vmod_q*cos(th_VSM[1])+Vmod_d*sin(th_VSM[1]);
...
//                 Vmod.As = temp_mod*cos(th_VSM[1]);
//                 Vmod.Bs = temp_mod*cos(th_VSM[1]-ALPHA);
//                 Vmod.Cs = temp_mod*cos(th_VSM[1]+ALPHA);
           ...
           th_VSM[0] = th_VSM[1];
           omega_VSM[0] = omega_VSM[1];
           omegadot_VSM[0] = omegadot_VSM[1];
