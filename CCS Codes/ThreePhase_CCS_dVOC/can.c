...
#define BW_cur 2*pi*2000 // 8kHz
#define BW_vol 2*pi*400 // 1kHz
...
//    configureDAC(1); //DAC_NUM
    ...
    LPF_Q = InitiateLPF( Tsamp, 62.8319); // 10 hz
        ...
        Vdot_dVOC[0] =0.0;
        Vdot_dVOC[1] =0.0;
        omega_dVOC[0] = omega_nom;
        omega_dVOC[1] = omega_nom;
        th_dVOC[0] = 0.1;
        th_dVOC[1] = 0.1;

        //eta   = (2 * PI *5) * Vnom * Vnom *0.5 / 5000; //original
        eta=44; //converged at this eta
        mu     = eta*5000/(0.05*pow(Vnom,4));//(2 * PI *0.5) / (0.95 * 0.95 * (1 - 0.95 * 0.95) * Vnom * Vnom);
...
interrupt void adca1_isr(void)
{
 //   GpioDataRegs.GPBDAT.bit.GPIO36 = 1; // - to check whether code fits or not
    ...
    PieCtrlRegs.PIEACK.all = PIEACK_GROUP1;
     ...
     INV_CURRENT_p.Ds =  (INV_CURRENT.As*cos(th_dVOC[1]) + INV_CURRENT.Bs*cos(th_dVOC[1]-ALPHA) + INV_CURRENT.Cs*cos(th_dVOC[1]+ALPHA))*0.66666667;
     INV_CURRENT_p.Qs = (-INV_CURRENT.As*sin(th_dVOC[1]) - INV_CURRENT.Bs*sin(th_dVOC[1]-ALPHA) - INV_CURRENT.Cs*sin(th_dVOC[1]+ALPHA))*0.66666667;
     ...
     CAP_VOLTAGE_p.Ds = (CAP_VOLTAGE.As*cos(th_dVOC[1]) + CAP_VOLTAGE.Bs*cos(th_dVOC[1]-ALPHA) + CAP_VOLTAGE.Cs*cos(th_dVOC[1]+ALPHA))*0.66666667;
     CAP_VOLTAGE_p.Qs = (-CAP_VOLTAGE.As*sin(th_dVOC[1]) - CAP_VOLTAGE.Bs*sin(th_dVOC[1]-ALPHA) - CAP_VOLTAGE.Cs*sin(th_dVOC[1]+ALPHA))*0.66666667;
     ...
     GRID_CURRENT_p.Ds =  (GRID_CURRENT.As*cos(th_dVOC[1]) + GRID_CURRENT.Bs*cos(th_dVOC[1]-ALPHA) + GRID_CURRENT.Cs*cos(th_dVOC[1]+ALPHA))*0.66666667;
     GRID_CURRENT_p.Qs = (-GRID_CURRENT.As*sin(th_dVOC[1]) - GRID_CURRENT.Bs*sin(th_dVOC[1]-ALPHA) - GRID_CURRENT.Cs*sin(th_dVOC[1]+ALPHA))*0.66666667;
     ...
     GRID_VOLTAGE_p.Ds = (GRID_VOLTAGE.As*cos(th_dVOC[1]) + GRID_VOLTAGE.Bs*cos(th_dVOC[1]-ALPHA) + GRID_VOLTAGE.Cs*cos(th_dVOC[1]+ALPHA))*0.66666667;
     GRID_VOLTAGE_p.Qs = (-GRID_VOLTAGE.As*sin(th_dVOC[1]) - GRID_VOLTAGE.Bs*sin(th_dVOC[1]-ALPHA) - GRID_VOLTAGE.Cs*sin(th_dVOC[1]+ALPHA))*0.66666667;
...
//     th_dVOC[1] = th_dVOC[0] + omega_nom*Tsamp;
//      if(th_dVOC[1]>2*PI)
//      {
//          th_dVOC[1] = th_dVOC[1]-2.0*PI;
//
//      }
//      else if(th_dVOC[1]<0)
//      {
...
//      }
    ...
    //    Q = 1.5*(-CAP_VOLTAGE_p.Ds*GRID_CURRENT_p.Qs  +  CAP_VOLTAGE_p.Qs*GRID_CURRENT_p.Ds);
       ...
       Q = LPFCalculation( 1.5*(-CAP_VOLTAGE_p.Ds*GRID_CURRENT_p.Qs  +  CAP_VOLTAGE_p.Qs*GRID_CURRENT_p.Ds) , &LPF_Q);
        ...
        {
             V_dVOC[1]   =  Vnom;
             Vgrid_Q[1] = GRID_VOLTAGE_p.Qs;
             int_Vgrid_Q[1] = int_Vgrid_Q[0] + 0.5*11.39*(Vgrid_Q[1] + Vgrid_Q[0])*Tsamp;
             omega_dVOC[1] = omega_nom  + int_Vgrid_Q[1] + 4.44*Vgrid_Q[1] ;
             int_Vgrid_Q[0] = int_Vgrid_Q[1];
             Vgrid_Q[0] =  Vgrid_Q[1];
            //  omega_dVOC[1] = omega_nom + presync_gain*GRID_VOLTAGE_p.Qs;
            // omega_dVOC[1] = omega_nom - presync_gain*sin(th_dVOC[1]- atan2(GRID_VOLTAGE_c.Be, GRID_VOLTAGE_c.Al));
        }
        else//if(RELAY==1)
        {
            err_Q[1] = Q_REF-Q;
            int_err_Q[1] = int_err_Q[0] + 0.5*ki_Q*(err_Q[1] + err_Q[0])*Tsamp;
            Vdot_dVOC[1] = mu*V_dVOC[1]*(Vnom*Vnom-V_dVOC[1]*V_dVOC[1]) + (0.66667*eta/V_dVOC[1])*err_Q[1]+int_err_Q[1];
            V_dVOC[1] = V_dVOC[0] + 0.5*(Vdot_dVOC[1] + Vdot_dVOC[0])*Tsamp;
            omega_dVOC[1] = omega_nom - (0.66667*eta/(V_dVOC[1]*V_dVOC[1]))*(P - P_REF);
            err_Q[0] = err_Q[1];
         ...
         th_dVOC[1] = th_dVOC[0] + 0.5*(omega_dVOC[1] + omega_dVOC[0])*Tsamp;
        if(th_dVOC[1]>2*PI)
        {
            th_dVOC[1] = th_dVOC[1]-2.0*PI;
...
//         VD_REF = V_dVOC[1] - Rv*(GRID_CURRENT_p.Ds - GRID_VOLTAGE_p.Ds/22.0);
//         VQ_REF = 0.0     - Rv*(GRID_CURRENT_p.Qs - GRID_VOLTAGE_p.Qs/22.0);
...
//next two are single loop. both are same. Be careful to check VD_REF is commented. V_droop is safe to use
  ...
  //E. single loop, use this or next, both same
         //Vmod_d     = (V_dVOC[1])/(0.5*Vdcnom);
         //Vmod_q     = 0.0 ;
 //F.
      // Vmod_d     = (VD_REF)/(0.5*Vdc);
      // Vmod_q     = 0.0 ;
             ...
             Vmod_alpha = Vmod_d*cos(th_dVOC[1])-Vmod_q*sin(th_dVOC[1]);
             Vmod_beta  = Vmod_q*cos(th_dVOC[1])+Vmod_d*sin(th_dVOC[1]);
           ...
           th_dVOC[0] = th_dVOC[1];
           omega_dVOC[0] = omega_dVOC[1];
           Vdot_dVOC[0] = Vdot_dVOC[1];
